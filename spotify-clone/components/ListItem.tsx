...
interface ListItemPorps {
    image: string;
    name: string;
    href: string;
}

const ListItem: React.FC<ListItemPorps> = ({
    image, name, href
}) => {
    ...
    return (
        <button onClick={onClick} className="relative group flex items-center rounded-md overflow-hidden gap-x-4 bg-neutral-100/10 hover:bg-neutral-100/20 transition pr-4">
            <div className="relative min-h-[64px] min-w-[64px]">
                <Image className="object-cover" fill src={image} alt="Liked Image" />
            </div>
            <p className="font-md truncate py-5">
                {name}
            </p>
